...
5つのシグナルを活用するとよく、まずは前半三つから使うと良い。
...
言わずとしれたログ。k8sにもデフォルトでログ機能が備わっていて、`kubectl logs`もその仕組み。しかしPodが削除されるなどするとログも一緒に削除されるため、本番運用にあたっては永続化の仕組みを取り入れる必要がある。

Fluentd, Fluentbit, Grafana Lokiなど、ログ転送とログ収集・検索を可能にするOSSがある。

##### Metrics

こちらもご存知メトリクス。測定値を使用して統計的な処理・分析を行うのに有用。k8sにはデフォルトでは収集機能がないため、外部サービスを利用する必要がある。

昨今だとPrometheus。

##### Traces

耳慣れない単語。ユーザーないしはアプリの通信を追跡するための概念。操作(Span)の集合から成る。複数のコンテナを経由するようなユーザーからのリクエストで問題が発生した場合、どのコンテナが原因かを辿る際に有用。また、各Spanでどれだけ時間がかかったのかも細かく可視化できる。

ただし、正確な情報を収集するために経路上全てを網羅する必要があるため、導入のハードルは高め。

アプリ内でTraceとSpanを実装する必要があるが、その際に役立つOSSといてOpenTelemetryがある。また、収集ではJaegerやGrafana Tempoなどがある。
...
- ダッシュボード
  - 収集したデータを観測可能にするように可視化する。Grafanaが該当。
- アラート
  - 異常を知らせる機能。Metricsを元に閾値を設定し、徐々にカスタムしていくのがよい。PrometheusのAlertManager。

#### モニタリング用システム構築

PrometheusとGrafanaを使ってモニタリング用システムを構築してみる。Metrics収集とダッシュボードによる可視化のみ。

詳細は本を買ってね。Helmでインストールした後のPodの状況。
...
メトリクス収集対象のアプリを立ち上げる。元になるマニュフェストファイルは作者が公開している[Githubリポジトリ](https://github.com/aoi1/bbf-kubernetes)にある。
...
次にメトリクスを収集するための設定をPrometheusに入れ込む。
...
実際にWebブラウザで`localhost:9090`アクセスするとPrometheusのダッシュボードが確認でき、`hello-server`のメトリックが収集できていることが確認できる。

同様にGrafanaにもアクセスしてみる。

```bash
$ kubectl port-forward service/kube-prometheus-stack-grafana -n monitoring 8081:80
```

別で動かしているZabbixのポートと被るので`8081`に変更。初期パスワードは変わっているようなので確認する。
